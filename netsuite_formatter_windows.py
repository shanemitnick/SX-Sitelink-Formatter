...
LOAD_PATH = r"D:\My Drive\sitelink_files"
# FILE PATH FOR FORMATTED .XLS FILES TO BE PUT INTO
SAVE_PATH = r"X:\NETSUITE\AUTOMATA\output_files"
        ...
        if file[0] != ".":
            if file.startswith("ManagementSummary"):
                ...
                try:
                    ...
                    message = "!!!!!!!!!!! NO DEPOSIT FILE FOR " + loc_info[1] + " ON DATE " + date + " !!!!!!!!!!!!!"
...
def save_docs_individual(all_documents):
    ...
    for key in all_documents.keys():
        end_id = all_documents[key][-1]['External ID'].iloc[0]
        start_id = all_documents[key][0]['External ID'].iloc[0]

        title = LOC_HYP[key] + "_starting_" + start_id + "_ending_" + end_id + '_netsuite_upload'
...
def save_docs_condensed_file(all_documents):
    """ Saves all of the documents into one file

    :param all_documents: Dictionary of documents desired to be saved.
    :return: null
    """
    list_of_docs = []
    # loop through each store
    for key in all_documents:
        # loop through each day for each store and add to master list
        for df in all_documents[key]:
            list_of_docs.append(df)
        end_id = all_documents[key][-1]['External ID'].iloc[0][1:]
        start_id = all_documents[key][0]['External ID'].iloc[0][1:]

    title = "MASTER_UPLOAD_" + start_id + "_ENDING_" + end_id
    store_file_df = pd.concat(list_of_docs)
    store_file_df.to_csv(f'{SAVE_PATH}\\{title}.csv', index=False)
...
def driver_code():
    ...
    save_docs_individual(documents)
    # this line is to save all files to one condensed file list.
    save_docs_condensed_file(documents)
